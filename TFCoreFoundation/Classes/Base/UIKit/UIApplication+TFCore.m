...
TFSYNTH_DUMMY_CLASS(UIApplication_TFCore)

#define kNetworkIndicatorDelay (1/30.0)
@interface _TFUIApplicationNetworkIndicatorInfo : NSObject
@property (nonatomic, assign) NSInteger count;
@property (nonatomic, strong) NSTimer *timer;
@end

@implementation _TFUIApplicationNetworkIndicatorInfo
@end

@implementation UIApplication (TFCore)
- (NSURL *)documentsURL {
...
TFSYNTH_DYNAMIC_PROPERTY_OBJECT(networkActivityInfo, setNetworkActivityInfo, RETAIN_NONATOMIC, _TFUIApplicationNetworkIndicatorInfo *);
...
- (void)_changeNetworkActivityCount:(NSInteger)delta {
    @synchronized(self){
        dispatch_async_on_main_queue(^{
            _TFUIApplicationNetworkIndicatorInfo *info = [self networkActivityInfo];
            if (!info) {
                info = [_TFUIApplicationNetworkIndicatorInfo new];
