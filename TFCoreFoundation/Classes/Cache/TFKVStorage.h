...
/**
 TFKVStorageItem is used by `TFKVStorage` to store key-value pair and meta data.
 Typically, you should not use this class directly.
 */
@interface TFKVStorageItem : NSObject
...
/**
 Storage type, indicated where the `TFKVStorageItem.value` stored.
 ...
 * If you want to store large number of small datas (such as contacts cache), 
   use TFKVStorageTypeSQLite to get better performance.
 * If you want to store large files (such as image cache),
   use TFKVStorageTypeFile to get better performance.
 * You can use TFKVStorageTypeMixed and choice your storage type for each item.
 
 See <http://www.sqlite.org/intern-v-extern-blob.html> for more information.
 */
typedef NS_ENUM(NSUInteger, TFKVStorageType) {
    
    /// The `value` is stored as a file in file system.
    TFKVStorageTypeFile = 0,
    
    /// The `value` is stored in sqlite with blob type.
    TFKVStorageTypeSQLite = 1,
    
    /// The `value` is stored in file system or sqlite based on your choice.
    TFKVStorageTypeMixed = 2,
};



/**
 TFKVStorage is a key-value storage based on sqlite and file system.
 Typically, you should not use this class directly.
 
 @discussion The designated initializer for TFKVStorage is `initWithPath:type:`. 
 ...
 */
@interface TFKVStorage : NSObject
...
@property (nonatomic, readonly) TFKVStorageType type;  ///< The type of this storage.
 ...
 */
- (nullable instancetype)initWithPath:(NSString *)path type:(TFKVStorageType)type NS_DESIGNATED_INITIALIZER;
...
/**
 ...
 If the `type` is TFKVStorageTypeFile, then the item.filename should not be empty.
 If the `type` is TFKVStorageTypeSQLite, then the item.filename will be ignored.
 It the `type` is TFKVStorageTypeMixed, then the item.value will be saved to file 
 system if the item.filename is not empty, otherwise it will be saved to sqlite.
 
 @param item  An item.
 @return Whether succeed.
 */
- (BOOL)saveItem:(TFKVStorageItem *)item;

/**
 Save an item or update the item with 'key' if it already exists.
 
 @discussion This method will save the key-value pair to sqlite. If the `type` is
 TFKVStorageTypeFile, then this method will failed.
...
/**
 ...
 @discussion
 If the `type` is TFKVStorageTypeFile, then the `filename` should not be empty.
 If the `type` is TFKVStorageTypeSQLite, then the `filename` will be ignored.
 It the `type` is TFKVStorageTypeMixed, then the `value` will be saved to file
 ...
 */
- (nullable TFKVStorageItem *)getItemForKey:(NSString *)key;
 ...
 */
- (nullable TFKVStorageItem *)getItemInfoForKey:(NSString *)key;
...
/**
 ...
 @param keys  An array of specified keys.
 @return An array of `TFKVStorageItem`, or nil if not exists / error occurs.
 */
- (nullable NSArray<TFKVStorageItem *> *)getItemForKeys:(NSArray<NSString *> *)keys;
...
/**
 ...
 @return An array of `TFKVStorageItem`, or nil if not exists / error occurs.
 */
- (nullable NSArray<TFKVStorageItem *> *)getItemInfoForKeys:(NSArray<NSString *> *)keys;
