...
CGContextRef _Nullable TFCGContextCreateARGBBitmapContext(CGSize size, BOOL opaque, CGFloat scale);

/// Create a `DeviceGray` Bitmap context. Returns NULL if an error occurs.
CGContextRef _Nullable TFCGContextCreateGrayBitmapContext(CGSize size, CGFloat scale);



/// Get main screen's scale.
CGFloat TFScreenScale();
 ...
 */
CGAffineTransform TFCGAffineTransformGetFromPoints(CGPoint before[3], CGPoint after[3]);

/// Get the transform which can converts a point from the coordinate system of a given view to another.
CGAffineTransform TFCGAffineTransformGetFromViews(UIView *from, UIView *to);
...
UIViewContentMode TFCAGravityToUIViewContentMode(NSString *gravity);

/// Convert UIViewContentMode to CALayer's gravity string.
NSString *TFUIViewContentModeToCAGravity(UIViewContentMode contentMode);
 ...
 */
CGRect TFCGRectFitWithContentMode(CGRect rect, CGSize size, UIViewContentMode mode);
...
// main screen's scale
#ifndef kScreenScale
#define kScreenScale TFScreenScale()
#endif

// main screen's size (portrait)
#ifndef kScreenSize
#define kScreenSize TFScreenSize()
#endif
