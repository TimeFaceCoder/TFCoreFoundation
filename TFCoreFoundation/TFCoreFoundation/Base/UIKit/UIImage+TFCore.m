...
+ (UIImage *)imageWithSmallGIFData:(NSData *)data scale:(CGFloat)scale {
    ...
    for (size_t i = 0; i < count; i++) {
        NSTimeInterval delay = _tf_CGImageSourceGetGIFFrameDelayAtIndex(source, i);
        totalTime += delay;
...
+ (UIImage *)imageWithPDF:(id)dataOrPath {
    return [self _tf_imageWithPDF:dataOrPath resize:NO size:CGSizeZero];
}

+ (UIImage *)imageWithPDF:(id)dataOrPath size:(CGSize)size {
    return [self _tf_imageWithPDF:dataOrPath resize:YES size:size];
}
...
+ (UIImage *)_tf_imageWithPDF:(id)dataOrPath resize:(BOOL)resize size:(CGSize)size {
...
+ (UIImage *)imageWithColor:(UIColor *)color alpha:(CGFloat)alpha {
    return [self imageWithColor:color size:CGSizeMake(1, 1) alpha:alpha];
}

+ (UIImage *)imageWithColor:(UIColor *)color size:(CGSize)size {
    return [self imageWithColor:color size:size alpha:1.f];
}
...
- (void)drawInRect:(CGRect)rect withContentMode:(UIViewContentMode)contentMode clipsToBounds:(BOOL)clips{
    CGRect drawRect = TFCGRectFitWithContentMode(rect, self.size, contentMode);
...
- (UIImage *)_tf_flipHorizontal:(BOOL)horizontal vertical:(BOOL)vertical {
...
- (UIImage *)imageByRotate180 {
    return [self _tf_flipHorizontal:YES vertical:YES];
}

- (UIImage *)imageByFlipVertical {
    return [self _tf_flipHorizontal:NO vertical:YES];
}

- (UIImage *)imageByFlipHorizontal {
    return [self _tf_flipHorizontal:YES vertical:NO];
}
...
- (UIImage *)imageByBlurRadius:(CGFloat)blurRadius
    ...
    if (!hasBlur && !hasSaturation) {
        return [self _tf_mergeImageRef:imageRef tintColor:tintColor tintBlendMode:tintBlendMode maskImage:maskImage opaque:opaque];
    }
    ...
    if (hasNewFunc) {
        ...
        effectCGImage = vImageCreateCGImageFromBuffer(input, &format, &_tf_cleanupBuffer, NULL, kvImageNoAllocate, NULL);
        if (effectCGImage == NULL) {
            effectCGImage = vImageCreateCGImageFromBuffer(input, &format, NULL, NULL, kvImageNoFlags, NULL);
            free(input->data);
        }
        free(output->data);
        outputImage = [self _tf_mergeImageRef:effectCGImage tintColor:tintColor tintBlendMode:tintBlendMode maskImage:maskImage opaque:opaque];
...
- (UIImage *)_tf_mergeImageRef:(CGImageRef)effectCGImage
                     tintColor:(UIColor *)tintColor
