...
@implementation UIDevice (TFCore)
...
- (BOOL)isJailbroken {
    ...
    NSString *path = [NSString stringWithFormat:@"/private/%@", [NSString tf_stringWithUUID]];
...
typedef struct {
    ...
    uint64_t awdl_in;
    uint64_t awdl_out;
} tf_net_interface_counter;


static uint64_t tf_net_counter_add(uint64_t counter, uint64_t bytes) {
...
static uint64_t tf_net_counter_get_by_type(tf_net_interface_counter *counter, TFNetworkTrafficType type) {
    uint64_t bytes = 0;
    if (type & TFNetworkTrafficTypeWWANSent) bytes += counter->pdp_ip_out;
    if (type & TFNetworkTrafficTypeWWANReceived) bytes += counter->pdp_ip_in;
    if (type & TFNetworkTrafficTypeWIFISent) bytes += counter->en_out;
    if (type & TFNetworkTrafficTypeWIFIReceived) bytes += counter->en_in;
    if (type & TFNetworkTrafficTypeAWDLSent) bytes += counter->awdl_out;
    if (type & TFNetworkTrafficTypeAWDLReceived) bytes += counter->awdl_in;
    return bytes;
}

static tf_net_interface_counter tf_get_net_interface_counter() {
    ...
    tf_net_interface_counter counter = {0};
        ...
        while (cursor) {
            if (cursor->ifa_addr->sa_family == AF_LINK) {
                ...
                if (name) {
                    ...
                    counter_in = tf_net_counter_add(counter_in, data->ifi_ibytes);
                    sharedInCounters[name] = @(counter_in);
                    
                    uint64_t counter_out = ((NSNumber *)sharedOutCounters[name]).unsignedLongLongValue;
                    counter_out = tf_net_counter_add(counter_out, data->ifi_obytes);
...
- (uint64_t)tf_getNetworkTrafficBytes:(TFNetworkTrafficType)types {
    tf_net_interface_counter counter = tf_get_net_interface_counter();
    return tf_net_counter_get_by_type(&counter, types);
}
