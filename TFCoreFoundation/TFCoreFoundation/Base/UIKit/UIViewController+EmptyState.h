 ...
 *  @brief viewState indicate which state the view is.
    ...
    If setter is required, use tf_showStateView:inView:, this will set the viewState implicitly. Each UIViewController can only have one viewState.
 */
...
/**
 *  remove current state view in view.
 *
 *  @param view super view of state view.
 */
- (void)tf_removeStateViewInView:(UIView *)view;

/**
 *  remove current state view in self.view.
 */
