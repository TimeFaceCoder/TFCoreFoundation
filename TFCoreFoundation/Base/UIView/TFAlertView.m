 ...
 *  石榴仔手部图片
 */
@property (nonatomic ,assign) TFAlertType         alertType;
/**
 *  弹出标题
 */
...
@property (nonatomic, copy) void (^TFAlertClickBlock)(NSInteger index);
...
- (id)initWithTitle:(NSString *)title
    ...
    self = [super init];
    if (self) {
        self.frame               = TFScreenBounds();
        self.title               = title;
        self.content             = content;
        self.alertType           = alertType;
        self.TFAlertClickBlock   = clickBlock;
        _contentWidth            = CGRectGetWidth(TFScreenBounds()) - kContentPadding*2 - kViewPadding*2;
        ...
        //
        UIImage *image = [[[[UIApplication sharedApplication] keyWindow] snapshotImage] imageByBlurLight];
        [self.blurImageView setImage:image];
        ...
        //线条
        UIView *line1 = [[UIView alloc] initWithFrame:CGRectMake(kContentPadding/2, 54, _contentView.tf_width - kContentPadding, .5)];
        line1.backgroundColor = TFSTYLEVAR(alertLineColor);
        [_contentView addSubview:line1];
        
        if (self.title.length) {
            self.titleLabel.text = self.title;
            self.titleLabel.tf_top = kContentPadding;
            self.titleLabel.tf_left = kContentPadding;
            self.titleLabel.tf_width = self.tf_width - kContentPadding * 2;
            [self.titleLabel sizeToFit];
            
            [_contentView addSubview:self.titleLabel];
        }
        CGFloat top = line1.tf_bottom + 20.f;
        if (self.content.length) {
            self.contentLabel.tf_top   = top;
            self.contentLabel.tf_left  = (_contentView.tf_width - _contentSize.width)/2;
            [self.contentLabel setTf_size:_contentSize];
            ...
            top = self.contentLabel.tf_bottom + 20;
        }
        
        if (self.alertType == TFAlertInfo) {
            CGFloat buttonWidth      = (_contentView.tf_width - kContentPadding*3)/2;
            self.firstButton.tf_size  = CGSizeMake(buttonWidth, kButtonHeight);
            self.firstButton.tf_left  = kContentPadding;
            self.firstButton.tf_top   = top;
            
            self.secondButton.tf_size = CGSizeMake(buttonWidth, kButtonHeight);
            self.secondButton.tf_left = _contentView.tf_width - kContentPadding - buttonWidth;
            self.secondButton.tf_top  = self.firstButton.tf_top;
            ...
            self.firstButton.tf_size = CGSizeMake(_contentView.tf_width - kViewPadding * 2, kButtonHeight);
            self.firstButton.tf_left = kViewPadding;
            self.firstButton.tf_top = top;
            [_contentView addSubview:self.firstButton];
        }
        self.contentView.tf_height = self.firstButton.tf_bottom + 20;
    }
    return self;
}
...
- (void)show {
    ...
    frame.origin.x = (CGRectGetWidth(TFScreenBounds()) - _contentView.tf_width ) / 2;
    frame.origin.y = (CGRectGetHeight(TFScreenBounds()) - _contentView.tf_height ) / 2;
...
- (UIView *)contentView {
    if (!_contentView) {
        _contentView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, CGRectGetWidth(TFScreenBounds()) - kViewPadding*2, 200)];
...
- (UIImageView *)blurImageView {
    if (!_blurImageView) {
        _blurImageView = [[UIImageView alloc] initWithFrame:TFScreenBounds()];
    }
...
- (UIButton *)firstButton {
    if (!_firstButton) {
        ...
        [_firstButton setBackgroundImage:[UIImage imageWithColor:TFSTYLEVAR(alertCancelColor)] forState:UIControlStateNormal];
        [_firstButton setBackgroundImage:[UIImage imageWithColor:TFSTYLEVAR(alertCancelHColor)] forState:UIControlStateHighlighted];
...
- (UIButton *)secondButton {
    if (!_secondButton) {
        ...
        [_secondButton setBackgroundImage:[UIImage imageWithColor:TFSTYLEVAR(alertOKColor)] forState:UIControlStateNormal];
        [_secondButton setBackgroundImage:[UIImage imageWithColor:TFSTYLEVAR(alertOKHColor)] forState:UIControlStateHighlighted];
...
- (void)onViewClick:(UIButton *)sender {
    ...
    if (self.TFAlertClickBlock) {
        self.TFAlertClickBlock(index);
    }
    [self dismiss];
}
